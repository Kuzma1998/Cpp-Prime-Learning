...
- `forward_list,list`不支持元素随机访问。
- `queue`是容器适配器，队列，尾部插入头部删除
- `string,vector`将元素存储在连续内存空间

## 容器操作
- 这些操作是所有类型都可以提供的，容器都是模板类
### 类型
...
- 如果元素是内置类型或者具有默认构造函数的类类型，可以只为构造函数提供一个大小参数，否则要提供一个显示初值。
```c++
array<int,42> ia;//要提供一个大小参数
```
...
- `list和forward_list和deqie`支持`push_front和emplace_front`
