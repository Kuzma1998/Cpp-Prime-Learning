...
- 标准库没有针对每个容器定义相应的成员函数来实现排序查找等操作，而是定义了一组泛型算法，所以称之为“**算法**”；而“**泛型**”、指的是它们可以操作在多种容器类型上。
...
```c++
//find_first_of，输入：两对迭代器标记两段范围，
//在第一段中找第二段中任意元素，返回第一个匹配的元素，找不到返回第一段的end迭代器。
auto iter = find(vec.begin(), vec.end(),target);//
int sum = accumulate(vec.begin(),vec.end(),0);// 求vec的和，0决定了accumulate的返回类型
bool res = equal(vec.begin(), vec.end(),rost.begin());//equal假定第二个序列至少于第一个一样长
```
...
- `fill_n`： `fill_n(vec.begin(), 10, 0);` vec开头的10各元素置为0
- 插入迭代器`back_inserter`：
  - 用来确保算法有足够的空间存储数据。
```C++
#include <iterator>
auto it = back_inserter(vec);//接受一个容器的引用，返回一个与改容器绑定的插入迭代器
*it = 42; //赋值运算调用push_back()插入vec

vector<int> vec;
fill_n(back_inserter(vec),10,0);// 正确，传递的参数时back_insert()返回的迭代器，直接插入10个0
```C++
- 拷贝算法`copy`：
- 输入：前两个参数指定输入范围，第三个指向目标序列。
- `copy (ilst.begin(), ilst.end(), back_inserter(ivec));`
- `copy`时必须保证目标目的序列至少要包含与输入序列一样多的元素。
- 返回位置迭代器递增后的值，即拷贝之后最后一个元素的后一个迭代器。
```c++
replace(ilist.begin(),ilist.end(),0,42);// 序列的0都换成42
replace_copy(ilist.begin(),ilist.end(),back_inserter(ivec),0,42);
// 原来的ilist不变，ivec包含ilist的拷贝并且把0换成了42
```
...
- 消除重复`unique`：
  ...
  - 顺序会变，重复的元素被“删除”。
  - 并没有真正删除，真正删除必须使用容器操作,后面的元素不知道变成啥了。
...
stable_sort(words.begin(), words.end(), isShorter);
// 默认从小到大，isShorter谓词，从大到小
```

### lambda表达式

4种可调用的对象：函数，函数指针，lambda表达式，重载了函数调用运算符的类。
- 有时可能希望操作可以接受更多的参数。
- `lambda`表达式表示一个可调用的代码单元，可以理解成是一个未命名的内联函数。
- 形式：`[capture list](parameter list) -> return type {function body}`。
  - 其中`capture list`捕获列表是一个`lambda`所在函数定义的局部变量的列表（通常为空，表示不捕获它所在函数种的任何局部变量）。不可忽略。
...
- 标准库`bind`函数：
  ...
  - `auto g = bind(f, a, b, _2, c, _1);`调用`g(_1, _2)`实际上调用`f(a, b, _2, c, _1)` 传递给g的参数按位置绑定到占位符 
  - 非占位符的参数可以拷贝传递也可以引用传递，要使用引用传参时，必须使用标准库`ref`函数或者`cref`函数。
```c++
for_each(words.begin(), words.end(),bind(print,ref(os),_1,' ')); // 不能传递os的拷贝，因此要加ref(os)
```
...
copy(lst.begin(), lst.end(),inserter(lst3,lst3.begin())); // 1,2,3,4 现在最开始插入一个元素，1，然后迭代器变成1后面那个迭代器，再在那个迭代器前面插入2。 1，2
```

### iostream迭代器

- 迭代器可与输入或输出流绑定在一起，用于迭代遍历所关联的IO 流。
...
```c++
istream_iterator<int> init(cin);// 定义一个流迭代器init，绑定到cin
istream_iterator<int> eof;//定义尾后迭代器
while(init!=eof){
  vec.push_back(*init++);// 从标准输入输出读取数据存入vec
}
...
```c++
ostream_iterator<int> out_iter(cout," ");// 定义一个流迭代器init，绑定到cout
for(auto i:vec){
  *out_iter++= i;//输出 
}

copy(vec.begin(),vec.end(),out_iter);//更简单
...
```c++
// 比如find_if(x,x,pred),remove_if(x,x,pred)第二个参数接受一个谓词
```
